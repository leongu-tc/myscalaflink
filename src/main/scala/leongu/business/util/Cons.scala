...
trait Cons {
  ...
  val KAFKA_STARTING_OFFSETS = "kafka_startingOffsets"
  ...
  val HBASE_TBL = "hbase_tbl"
  val HBASE_NAMESPACE = "hbase_ns"

  val SYNC_DAY = "sync_day"
  val BULKLOAD_DIR = "bulkload_dir"

  val QUERY = "query"
  val CHECKPOINT_LOCATION = "checkpoint_location"
  // cons
  ...
  // FLINK CONF
  val CP_INTERVAL = ExecutionCheckpointingOptions.CHECKPOINTING_INTERVAL.key()
  val CP_MODE = ExecutionCheckpointingOptions.CHECKPOINTING_MODE.key()
  val EOS = CheckpointingMode.EXACTLY_ONCE
  val ALOS = CheckpointingMode.AT_LEAST_ONCE
  val MIN_PAUSE_CP = ExecutionCheckpointingOptions.MIN_PAUSE_BETWEEN_CHECKPOINTS.key()
  val MAX_CURR_CP = ExecutionCheckpointingOptions.MAX_CONCURRENT_CHECKPOINTS.key()
}
